<div class="min-h-screen bg-gray-50">
    ...
    @if($mainSlug === 'meserii')
        <div x-data="{ open: false }">
            ...
            <div x-show="open" 
                 ...
                 class="mt-2">
                <div class="grid grid-cols-2 gap-1 p-2 rounded-lg bg-gray-50 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                                                ...
                                                @if(isset($subCategory['subcategories']) && !empty($subCategory['subcategories']))
                                                    <div x-show="open" 
                                                        ...
                                                        @foreach($subCategory['subcategories'] as $subSubSlug => $subSubCat)
                                                            <button wire:click="$set('categorySlug', '{{ $subSubSlug }}')"
    ...
    @if ($script || $audioUrl || $imageUrl || $videoUrl || $isProcessing)
        <div class="mt-8 space-y-8">
            ...
            @if ($isProcessing)
                <div wire:poll.5s="checkStatus" class="mt-8">
                    <div class="p-6 bg-white shadow-sm rounded-xl">
                        <h2 class="flex items-center gap-2 mb-4 text-xl font-bold text-gray-900">
                            <svg class="w-5 h-5 text-purple-500" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
